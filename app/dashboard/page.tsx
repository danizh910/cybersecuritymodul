...
export default function DashboardPage() {
  ...
  async function startGeo() {
    ...
    return new Promise<void>((resolve) => {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          patchModule({ moduleId: 'geolocation', status: 'nicht erlaubt', details: {} });
          resolve();
        }
      );
    });
  ...
  async function startCamMic() {
    ...
    try {
      ...
      patchModule({ moduleId: 'camera-microphone', status: 'nicht erlaubt', details: { streamFreigegeben: false } });
    }
  }
  ...
  async function startFonts() {
    ...
    try {
      ...
      patchModule({ moduleId: 'local-fonts', status: 'nicht erlaubt', details: {} });
    }
  }
  ...
  return (
    <div className="space-y-6">
      <header className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-sm">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">Was sieht eine Webseite ueber mich?</h1>
      ...
      {mode === 'extended' && (
        <section className="space-y-3">
          ...
          <PermissionModule title="Modul 1: Geolocation Demo" warning="Standort ist hochsensitiv. Nur nach expliziter Freigabe." onStart={startGeo}><pre className="text-xs">{JSON.stringify(modules.find((m) => m.moduleId === 'geolocation'), null, 2)}</pre></PermissionModule>
          <PermissionModule title="Modul 2: Kamera/Mikrofon Demo" warning="Keine Aufzeichnung, kein Upload, nur lokale Abfrage." onStart={startCamMic}><pre className="text-xs">{JSON.stringify(modules.find((m) => m.moduleId === 'camera-microphone'), null, 2)}</pre></PermissionModule>
          <PermissionModule title="Modul 3: MediaDevices Demo" warning="Zeigt nur Anzahl/Typen, keine persistente deviceId-Nutzung." onStart={startMediaDevices}><pre className="text-xs">{JSON.stringify(modules.find((m) => m.moduleId === 'media-devices'), null, 2)}</pre></PermissionModule>
          <PermissionModule title="Modul 4: Local Font Access Demo" warning="Fonts koennen identifizierend sein; hier nur Anzahl + Beispiele lokal." onStart={startFonts}><pre className="text-xs">{JSON.stringify(modules.find((m) => m.moduleId === 'local-fonts'), null, 2)}</pre></PermissionModule>
          <PermissionModule title="Modul 5: WebRTC Transparency Demo" warning="Nur defensiv-minimale Transparenz ohne aggressive ICE-Sammlung." onStart={startWebRTC}><pre className="text-xs">{JSON.stringify(modules.find((m) => m.moduleId === 'webrtc'), null, 2)}</pre></PermissionModule>
        </section>
      )}
